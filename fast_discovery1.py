...
class FastDiscovery (EventMixin):
  ...
  def _handle_openflow_PortStatus (self, event):
    if (event.deleted or (event.ofp.desc.state & OFPPS_LINK_DOWN)):
         #the port is down#S
         log.debug("status of port %s have changed to or is down" % str(event.port))
         expired = [link for link in self.up
                       if (link.port1 == event.port) or (link.port2 == event.port)]
         for link in expired:
           ...
           #the port is up#
          log.debug("status of port %s have changed to or is up" % str(event.port))
