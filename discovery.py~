...
class LLDPSender (object):
  ...
  def _handle_openflow_PortStatus (self, event):
    ...
    """
    if event.added:
      ##print ("From normal discovery:PortStatus  up in %s" % event.port)
      self.add_port(event.dpid, event.port, event.ofp.desc.hw_addr)
    elif event.deleted:
      ##print ("From normal discovery:PortStatus  down in %s" % event.port)
      self.del_port(event.dpid, event.port)
    ##elif event.modified:
      ##print ("From normal discovery:PortStatus changed in %s" % event.port)
...
class Discovery (EventMixin):
  ...
  _core_name = "slow_discovery" # we want to be core.openflow_discovery
  ...
  """def _handle_openflow_PortStatus (self, event):
    ...
    if event.added:
      print ("PortStatus up in %s" % event.port)
      self.add_port(event.dpid, event.port, event.ofp.desc.hw_addr)
    elif event.deleted:
      print ("PortStatus down in %s" % event.port)
      self.del_port(event.dpid, event.port)
    elif event.modified:
      print ("PortStatus changed in %s" % event.port)
    """
  def _handle_openflow_ConnectionUp (self, event):
  ...
  def _handle_openflow_ConnectionDown (self, event):
    ...
    log.debug("Deleting flow for %s", dpid_to_str(event.dpid))
  ...
  def _expire_links (self):
    ...
    if expired:
      ...
      self.raiseEventNoErrors(ExpireLink, link)     ## Luyi Guo
  ...
  def _handle_openflow_PacketIn (self, event):
    ...
    if (packet.effective_ethertype != pkt.ethernet.LLDP_TYPE
      ...
      ##print "Inside halt if: "+str((packet.effective_ethertype != pkt.ethernet.LLDP_TYPE, packet.dst != pkt.ETHERNET.NDP_MULTICAST))
      ##print "Var: "+str((packet.effective_ethertype, pkt.ethernet.LLDP_TYPE, packet.dst, pkt.ETHERNET.NDP_MULTICAST))
    ...
    if link not in self.adjacency:
      ...
      log.debug('link detected: %s.%i -> %s.%i' %
               (dpid_to_str(link.dpid1), link.port1,
                dpid_to_str(link.dpid2), link.port2))
